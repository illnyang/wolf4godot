...
func _apply_font(label: Label, font_id: int) -> void:
	...
	if font:
		...
		var final_scale = scale_factor * 0.7
...
func _update_difficulty_face() -> void:
	...
	if face_rect and difficulty_index < difficulty_options.size():
		...
		if pics.has(pic_key):
			...
			var offset_x = 50
			var centered_x = (160 - (tex.get_width() / 2.0)) + offset_x
			face_rect.position = Vector2(center_offset_x + centered_x * scale_factor, center_offset_y + 105 * scale_factor)
...
func _update_cursor() -> void:
	...
	match current_state:
		...
		MenuState.DIFFICULTY_SELECT:
			target_x = center_offset_x + 70 * scale_factor
			target_y = center_offset_y + (77 + difficulty_index * 26) * scale_factor
		MenuState.MAP_SELECT:
			target_x = center_offset_x + 80 * scale_factor
			target_y = center_offset_y + (49 + map_index * 14) * scale_factor
		MenuState.SAVE_GAME:
